...
export const GET: APIRoute = async (context) => {
  try {
    ...
    if (
      assigneeIdParam &&
      !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(assigneeIdParam)
    ) {
      return new Response(
        JSON.stringify({
  ...
  try {
    ...
    try {
      ...
      if (errorMessage === 'CATALOG_ITEM_NOT_FOUND') {
        return new Response(
          JSON.stringify({ error: 'Chore catalog item not found or not accessible' }),
          {
            status: 400,
            headers: { 'Content-Type': 'application/json' },
          }
      ...
      if (errorMessage === 'ASSIGNEE_NOT_IN_HOUSEHOLD') {
        return new Response(
          JSON.stringify({ error: 'Assignee does not belong to this household' }),
          {
            status: 400,
            headers: { 'Content-Type': 'application/json' },
          }
      ...
      if (errorMessage === 'DUPLICATE_CHORE') {
        return new Response(
          JSON.stringify({
            error: 'Duplicate chore already exists for this date, catalog item, assignee, and time',
          }),
          {
            status: 409,
            headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    try {
      ...
      if (errorMessage === 'UNAUTHORIZED') {
        return new Response(
          JSON.stringify({ error: 'Only household admin can delete chores by date' }),
          {
            status: 403,
            headers: { 'Content-Type': 'application/json' },
          }
