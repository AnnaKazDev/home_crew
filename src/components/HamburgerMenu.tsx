            ...
            >
              <div className="flex flex-col h-full p-6">
                ...
                <nav className="flex flex-col space-y-3 pt-12 flex-1">
                  {menuItems
                    .filter((item) => !(typeof item.label === 'string' && item.label === 'Sign out'))
                    .map((item, index) =>
                      item.separator ? (
                ...
                {menuItems.find((item) => typeof item.label === 'string' && item.label === 'Sign out') && (
                  <div className="mt-4">
                    <button
                      className="flex items-center gap-3 text-base font-medium text-foreground hover:text-primary transition-colors duration-200 px-4 py-2 rounded-md hover:bg-primary/5 text-left w-full"
                      onClick={() =>
                        handleItemClick(menuItems.find((item) => typeof item.label === 'string' && item.label === 'Sign out')!)
                      }
                    >
