...
const supabaseUrl =
  import.meta.env.PUBLIC_SUPABASE_URL || import.meta.env.SUPABASE_URL || 'http://127.0.0.1:54321';
...
export function getSupabaseClient(): ReturnType<typeof createClient<Database>> {
  ...
  if (!cachedBrowserClient) {
    const keyToUse =
      isServer && supabaseServiceKey ? supabaseServiceKey : (supabaseAnonKey as string);
